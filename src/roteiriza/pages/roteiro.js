...
const Roteiro = ({ viagemId }) => {
  ...
  const loadDados = async (viagemId) => {
    try {
      ...
      const passeioQuery = query(
        passeioCollectionRef,
        where("viagemId", "==", viagemId)
      );
      const alimentacaoQuery = query(
        alimentacaoCollectionRef,
        where("viagemId", "==", viagemId)
      );
  ...
  }, [selectedDate, listDados]);

  const filtro = (selectedDate) => {
    const filteredListDados = listDados.filter(
      (item) => item.data === selectedDate
    );
    const ListManha = filteredListDados.filter(
      (item) => item.Horario === "Manha"
    );
    const ListTarde = filteredListDados.filter(
      (item) => item.Horario === "Tarde"
    );
    const ListNoite = filteredListDados.filter(
      (item) => item.Horario === "Noite"
    );
